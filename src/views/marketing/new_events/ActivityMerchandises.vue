        ...
        <template slot = "header" slot-scope = "{ search, searchHandler }">
            <merchandise-header v-model = "search" @search = "searchHandler">
                <template slot = "opt-buttons">
                    ...
                    <add-merchandise :needUploadImage = "true" :show = "showAddMerchandise" @close="closeMerchandiseForm" />
                </template>
            </merchandise-header>
        </template>
        <template slot = "table" slot-scope = "{ data }">
            <merchandise-table :merchandises = "data">
                <template slot = "tableOpt" slot-scope = "{merchandise}">
                    <el-button type="text" size="mini" @click="edit(merchandise)" :disabled = "true">编辑</el-button>
        ...
        methods: {
            ...
            async closeMerchandiseForm(merchandise) {
                ...
                if (merchandise['merchandise_id'] && merchandise['stock_num'] && merchandise['main_image']) {
                    ...
                    merchandise = await this.http.activityMerchandises.create(this.$requestInput('projectId'), this.activity.id, data);
                }
