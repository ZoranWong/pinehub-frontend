    ...
    <el-form :model="activity" label-width="120px" ref="activity">
        ...
        <el-tabs active-name="first" type="card" style="margin-left:20px">
            <el-tab-pane label="基础应用" name="first">
                <el-button @click="activityType=0" size="" plain>{{ types[0] }}</el-button>
                <el-button @click="activityType=1" size="" disabled plain>{{ types[1] }}</el-button>
                <el-button @click="activityType=2" size="" disabled plain>{{ types[2] }}</el-button>
                <el-button @click="activityType=3" size="" disabled plain>{{ types[3] }}</el-button>
            </el-tab-pane>
        </el-tabs>
        <div style="margin:20px 0 0 20px">活动设置</div>
        <div style="margin-left:20px">用户付款完成后，可以 {{ types[activityType] }}：
            <el-button v-if = "activityType == 0 && tickets.length === 0" size="" type="text">添加优惠券</el-button>
            <el-form-item label="" v-else-if = "activityType == 0" prop="ticket_id" label-width="5px" style="display: inline-block;margin-bottom:0;width:40%;">
            ...
            <el-button v-if="activityType == 1" size="" type="text">添加优惠码</el-button>
            <el-button v-if="activityType == 2" size="" type="text">添加赠品</el-button>
            <el-button v-if="activityType == 3" size="" type="text">添加营销活动</el-button>
        </div>
    </el-form>
</template>
<script>
