<template>
    <el-form :model = "card"  label-width = "120px" ref = "memberCard" class = "member-card-info">
        ...
        <el-form-item label = "卡片封面：" prop = "cover"
                      :rules = "[{required: true, message: '请选择封面', trigger: 'blur'},{validator: coverValidator, message: '请选择封面颜色或者图片'}]">
                ...
                <div class = "card-background">
                    ...
                    <el-popover placement = "bottom" width = "64" trigger = "click" :disabled = "card['cover'] !== 'color'">
                        <div class = "color-picker">
                            <p v-for = "(color, index) in colors" @click = "selectColor(color)" :class = "color.key" :key = "index"></p>
                        </div>
                        <div class = "color-box" slot = "reference" size = "small">
                            <p :class = "bgColorClass" >
        ...
        <el-form-item label = "会员卡名称：" prop = "base_info.title"
                      :style = "card['base_info']['title'] ?'margin-bottom: 0;' : 'margin-bottom:15px'"
                      :rules = "[{required: true, message: '请填写会员卡名称', trigger: 'blur'}]">
            ...
            <el-checkbox-group v-model = "card['member_private']" @change = "changeMemberPrivate">
                ...
                <div class = "member-discount">
                    ...
                    <el-form-item label = "" label-width="10px" prop = "discount" style = "display: inline-block;">
                        <el-input-number size="small"
                                         v-model.number = "card['discount']" style = "width:124px;margin:0 10px;"
                                         :min = '1' :max = "10" type = "number"
                                         :disabled = "!hasMemberPrivate('discount')">
                        </el-input-number> 折
                    </el-form-item>
                </div>
                ...
                <div class = "score">
                    ...
                    <el-form-item label = "" label-width = "10px" prop = "bonus_rule.init_increase_bonus" style = "display: inline-block;">
                        <el-input-number size="small"
                                         v-model.number = "card['bonus_rule']['init_increase_bonus']"
                                         style = "width:124px;" type = "number" :disabled = "!hasMemberPrivate('score')">
                        </el-input-number> 积分
                    </el-form-item>
                </div>
            </el-checkbox-group>
        ...
        <el-form-item label = "使用须知：" prop = "base_info.description">
            <el-input v-model = "card['base_info']['description']" type = "textarea" :rows = "3" :maxlength = '300'
                      placeholder = "会员特权会跟你您上文的勾选情况系统自动生成，此处可填写其他补充信息以便会员知晓，最多可输入300个字符">
            </el-input>
        </el-form-item>
                ...
                <div class = "date-info-time-range" v-else>
                    <el-radio label = "DATE_TYPE_FIX_TIME_RANGE" :disabled = "card['base_info']['date_info']['fixed_term'] ? true : false">
                        <el-date-picker value-format = "yyyy-MM-dd HH:mm:ss"
                                        :disabled = "card['base_info']['date_info']['type']!== 'DATE_TYPE_FIX_TIME_RANGE' || card['base_info']['date_info']['fixed_term'] ? true : false"
        ...
        <el-form-item label = "过期设置：" prop = "over_date_setting">
            ...
            <el-button size = "small"  type = "text" :disabled = "true">刷新</el-button> |
            <el-button size = "small"  type = "text" :disabled = "true">新建</el-button>
        </el-form-item>
...
<script>
    ...
    export default {
        ...
        computed: {
            bgColorClass() {
                let color = _.findWhere(this.colors, {key: this.card['base_info']['color']});
                return color['key'];
            },
            bgColor() {
                let color = _.findWhere(this.colors, {key: this.card['base_info']['color']});
        ...
        methods: {
            selectColor(color) {
                this.card['base_info']['color'] = color['key'];
            },
            changeMemberPrivate() {
            ...
            coverValidator(rule, value, callback) {
                if(!this.card['cover'] || (this.card['cover'] === 'color' && !this.card['base_info']['color']) || (this.card['cover'] === 'pic'
