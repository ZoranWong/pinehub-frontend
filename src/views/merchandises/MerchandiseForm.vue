<template>
    <el-form ref="merchandise" :model="merchandise" label-width="120px">
        <h4>基本信息</h4>
        <el-form-item label="商品名：" prop="name" :rules = "[{required: true, message: '请填写商品名称', trigger: 'blur'}]">
            <el-input v-model="merchandise['name']" style="width: 256px" />
        </el-form-item>
        <el-form-item label="商品简介：" prop="preview" style="margin-bottom: 12px;" :rules = "[{required: true, message: '请填写商品简介', trigger: 'blur'}]">
            <el-input v-model="merchandise['preview']" placeholder = "微信分享给好友时会显示，建议255个字以内" style="width: 720px;" :maxlength="255" type = "textarea" :rows = "5" />
        </el-form-item>
    ...
    methods: {
        ...
        removeImages: function (rfile) {
            _.map(this.imageList, (file, index) => {
                if (file && (file.uid === rfile.uid || file.url === rfile.url)) {
                    this.imageList.splice(index, 1);
                    if (this.merchandise['images']) {
                        this.merchandise['images'].splice(index, 1);
                    }
                }
            })
        },
