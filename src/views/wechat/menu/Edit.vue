    ...
    export default {
        ...
        methods: {
            saveMenu(){
                ...
                this.$command('UPDATE_WECHAT_MENU', this.id, menus);
            }
        },
        created() {
            let menu = this.json.decode(this.base64.decode(this.$requestInput('menu')));
            this.id = menu.id;
            let menus = menu['menus']['button'];
            let count = menus.length;
            for(let i = count - 1; i >= 0; i --) {
                this.menu.menus.unshift(menus[i]);
            }
        }
    }
</script>
