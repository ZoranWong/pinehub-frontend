	...
	<div class="content-scroll">
		<div class="content-box" style="padding:20px 30px">
				...
				<el-row :gutter="20" class="toolbar gzhinfo">
					<el-col :span="12">
				  	...
				  	<el-col :span="11">
						...
						<div class="gzhPic">
							<img :src="IMAGE_SERVER_HOST+'/material/view?material_src='+detailData.qr_code" style="height:144px;width:144px" alt="" />
							<p>公众号二维码</p>
						</div>
				  	</el-col>
				</el-row>	
			...
			<el-dialog :visible.sync="replyVisible" @close="dialogClose" @open="dialogOpen" width="100%" :fullscreen="true" :modal="false" :top="scrollTop" :close-on-click-modal="false">
				<el-tabs v-model="replyName" @tab-click="replyClick">
				    <el-tab-pane label="关键词回复" name="first" class="layout">
				    	...
				    	<div class="form-container" v-if="keyValue && !addRule">
							...
							<el-table :data="keyList" highlight-current-row v-loading="tLoading" v-if="!keyDetail">
								...
								<el-table-column prop="content" label="回复内容" min-width="120" header="center">
									<template slot-scope="scope">
										<p v-if="scope.row.type=='image'">1图片</p>
										<p v-if="scope.row.type=='news'">1图文</p>
										<p v-if="scope.row.type=='video'">1视频</p>
										<p v-if="scope.row.type=='voice'">1语音</p>
										<p v-if="scope.row.type=='text'">{{scope.row.content}}</p>
									</template>
								</el-table-column>
							...
							<el-form v-loading="fLoading" label-width="100px" v-else style="border:1px solid #dfe6ec;border-radius: 5px;">
								...
								<el-form-item label="回复内容：">
									...
									<p v-if="keyLists.type=='image'"><img :src="IMAGE_SERVER_HOST+'/material/'+keyLists.content+'?app_id='+appId+'&token='+token+'&selected_appid='+appInfo.id" alt="" /></p>
									<p v-if="keyLists.type=='news'">1图文</p>
									<p v-if="keyLists.type=='video'">1视频</p>
									<p v-if="keyLists.type=='voice'">1语音</p>
									<p v-if="keyLists.type=='text'">{{keyLists.content}}</p>
								</el-form-item>
								<el-form-item style="float:right">
									...
									<div class="keyLayout">
										<div v-if="keyImgTextUrl && keyAllContent" class="keyContent">
											<el-card :body-style="{ padding: '0px',display:'inline-block' }">
											 	<div style="padding: 10px;"><time class="time">{{ keyImgTextUrl.update_time?timestampToTime(keyImgTextUrl.update_time):'' }}</time></div>
						...
						<div class="form-container" v-if="stopValue">
							...
							<div class="stopLayoutImg" v-else>
								...
								<div class="showImg" v-if="stopAddData.type=='image' && !stopImgShow">
									...
									<el-button icon="el-icon-edit" circle size="mini" @click="getImgMaterials('news')"></el-button>
								...
								<div class="showImg" v-if="stopAddData.type=='voice' && !stopVoiceShow">
									...
									<el-button icon="el-icon-edit" circle size="mini" @click="getVoiceMaterials('news')"></el-button>
								...
								<div class="showImg" v-if="stopAddData.type=='video' && !stopVideoShow">
									...
									<el-button icon="el-icon-edit" circle size="mini" @click="getVideoMaterials('news')"></el-button>
						...
						<div class="form-container" v-if="followValue">
							...
							<div class="stopLayoutImg" v-else>
								...
								<div class="showImg" v-if="followAddData.type=='image' && !followImgShow">
									...
									<el-button icon="el-icon-edit" circle size="mini" @click="getImgMaterials('followed')"></el-button>
								...
								<div class="showImg" v-if="followAddData.type=='voice' && !followVoiceShow">
									...
									<el-button icon="el-icon-edit" circle size="mini" @click="getVoiceMaterials('followed')"></el-button>
								...
								<div class="showImg" v-if="followAddData.type=='video' && !followVideoShow">
									...
									<el-button icon="el-icon-edit" circle size="mini" @click="getVideoMaterials('followed')"></el-button>
								...
								<el-form-item label="" prop="key" v-if="menuAddData.type=='click' && !submenuType" class="el-form-layout">
									...
									<div class="stopLayoutText" :style="!menuImgText?'':stopTextValue!=1?'line-height:280px;':''">
										...
										<div v-if="stopTextValue==1 && !menuImgText">
											<el-card :body-style="{ padding: '0px',display:'inline-block' }">
											 	<div style="padding: 10px;"><time class="time">{{ menuImgTextUrl.update_time?timestampToTime(menuImgTextUrl.update_time):'' }}</time></div>
										    ...
										    <el-button icon="el-icon-edit" circle size="mini" @click="getImgTextMaterials('menus')"></el-button>
										...
										<div v-if="stopTextValue==2 && !menuImg">
											...
											<el-button icon="el-icon-edit" circle size="mini" @click="getImgMaterials('menus')"></el-button>
										...
										<div v-if="stopTextValue==3 && !menuVoice">
											...
											<el-button icon="el-icon-edit" circle size="mini" @click="getVoiceMaterials('menus')"></el-button>
										...
										<div v-if="stopTextValue==4 && !menuVideo">
											...
											<el-button icon="el-icon-edit" circle size="mini" @click="getVideoMaterials('menus')"></el-button>
				...
				<div class="form-container">
					...
					<div class="imgTextContent">
						 <el-card :body-style="{ padding: '0px',position:'relative',display:'inline-block' }" v-for="(imgText, index) in imgTextDatas" :key="index" @click.native="chooseImgText(index,imgText)">
						 	...
						 	<div style="padding: 10px;"><time class="time">{{ imgText.update_time?timestampToTime(imgText.update_time):'' }}</time></div>
...
//						http://image.test.kingdomcloud.cn/material/nsZuM5IRDFadCMPrq2h4nRZzrqPKKmkqARusezpM-Yo?app_id=wx581a7ad7ca810da6&token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOjEsImlzcyI6Imh0dHA6Ly9hcGkuYXV0aC50ZXN0Lmtpbmdkb21jbG91ZC5jbi9hdXRoIiwiaWF0IjoxNTMyNjgzOTAyLCJleHAiOjE1MzI2ODc1MDIsIm5iZiI6MTUzMjY4MzkwMiwianRpIjoiTERWMlpSaGdXVUFKSjRPNiJ9.Dz7yOllfpsf1hFnq5Bgr6N3GS0TWlowsmF27EFEMYjw&selected_appid=kdy5b40274b6a29b
						this.keyImgUrl=this.IMAGE_SERVER_HOST+'/material/'+this.keyList[index].content+'?app_id='+this.appId+'&token='+JSON.parse(sessionStorage.getItem('token')).token+'&selected_appid='+this.appInfo.id
						this.keyText=''
						this.keyVideoName=''
						this.keyVoiceName=''
						this.keyImgTextUrl=''
						...
						this.getAssignMaterial(this.keyList[index].content)
						this.keyAddData.content=this.keyList[index].content
...
//		    			this.button[this.menuNum].media_id=this.videoSaveTo.media_id
			...
			replyClick(val){
				if(val.name=="second"){
//					console.log(this.stopAddData.type)
				}else if(val.name=="third"){
//					console.log(this.followAddData.type)
				}else{
				...
				if(valid){
					...
					if(result) {
				    	...
				    	if(list){
				    		...
				    		this.getReply(false,this.detailData)
				    	}
				    }
			    }
		    },
		    async attentionReply(data){
		    	let para={
		    		app_id:this.appId,
		    		type:data.type,
		    		focus_reply:data.focus_reply?data.focus_reply:false,
				...
				if(para.content){
					...
					if(result) {
						let list=''
						if(data.id){
							list = await this.adminApi(NoPublicService).replyUpdateData (data.id,para)
						}else{
							list = await this.adminApi(NoPublicService).replyCreateData (para)
						}
				    	if(list) {
							this.$message({message:"保存成功",type: 'success'})
						}
				...
				if(list){
					...
					for(var i in list){
						if(list[i].prefect_match_keywords || list[i].semi_match_keywords){
							...
							this.followAddData.id=list[i].id
							if(list[i].type=="image"){
								this.followImgShow=false
								this.followImgUrl=this.IMAGE_SERVER_HOST+'/material/'+list[i].content+'?app_id='+this.appId+'&token='+JSON.parse(sessionStorage.getItem('token')).token+'&selected_appid='+this.appInfo.id
//								this.followImgUrl=this.IMAGE_SERVER_HOST+'/material/'+list[i].content+'?app_id='+this.appId
								this.followAddData.content=''
							...
							this.stopAddData.id=list[i].id
							if(list[i].type=="image"){
								this.stopImgShow=false
								this.stopImgUrl=this.IMAGE_SERVER_HOST+'/material/'+list[i].content+'?app_id='+this.appId+'&token='+JSON.parse(sessionStorage.getItem('token')).token+'&selected_appid='+this.appInfo.id
//								this.stopImgUrl=this.IMAGE_SERVER_HOST+'/material/'+list[i].content+'?app_id='+this.appId
								this.stopAddData.content=''
							}else if(list[i].type=="video"){
								this.stopVideoShow=false
			...
			async getMenu(type,row){
				...
				if( result ) {
					...
					if(list){
						this.releaseId=list.id
						this.button=list[list.length-1].menus.button
