<template>
    <div class="tab-pane">
        <el-table highlight-current-row v-loading="isLoading" class="table-header" :data="orders" :span-method="spanMethod">
            ...
            <el-table-column prop="" label="售后" min-width="80">
                <template slot-scope="scope">
                    <el-button @click="refund(scope.row)">退款申请</el-button>
                </template>
            </el-table-column>
            <el-table-column prop="" label="自提卖家" >
                <template slot-scope="scope">
                    <div v-if="scope.row.item.shop && scope.row.item.shop && scope.row.item.shop.name">
