        ...
        <search-header @search="search">
            ...
            <template slot="opt">
                <div>
                    ...
                    <el-button size="small" icon="el-icon-download" @click="popUpExportOptionSelector">导出</el-button>
                </div>
            </template>
        </search-header>

        <el-dialog title="数据导出项选择" :visible.sync="fieldOptionVisible">
            <el-date-picker v-model="fieldOptions.paid_at" type="daterange" value-format="yyyy-MM-dd">
            </el-date-picker>
            <el-transfer v-model="fieldOptions.fields" :data="fieldOptionsAvailable"></el-transfer>
            <div slot="footer" class="dialog-footer">
                <el-button @click="fieldOptionVisible = false">取 消</el-button>
                <el-button type="primary" @click="exportHandler">确 定</el-button>
            </div>
        </el-dialog>
    </div>
</template>
    ...
    export default {
        ...
        data() {
            return {
                ...
                fieldOptionVisible: false,
                fieldOptionsAvailable: [
                    {
                        key: 'shop_name',
                        label: '店铺名称',
                        disable: false,
        ...
        methods: {
            ...
            popUpExportOptionSelector() {
                this.fieldOptionVisible = true;
            },
            async exportHandler() {
                let downloadUrl = await this.$command('EXCEL_EXPORT', this.$requestInput('projectId'), 'shop_turnover', {fieldOptions: this.fieldOptions});
