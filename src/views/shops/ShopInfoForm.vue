<template>
    <div class="form-container shop-info">
        <el-form ref="shopForm" :model="shop" label-width="120px">
            <el-form-item label="店铺编号：" prop="code"
                          :rules="[{ pattern: /^[0-9A-Za-z]{6,16}$/, message: '编号有大小写字母与数组组成，长度6-10', trigger: 'blur' }]">
                <!--                <el-input v-model="shop.code" style="width:30%" placeholder="输入已有的编号或者系统默认" :disabled="!!shop.id" />-->
                <el-input v-model="shop.code" style="width:30%" placeholder="输入已有的编号或者系统默认"/>
            </el-form-item>
            <el-form-item label="店铺名称："
                          prop="name"
                          :rules="[{ required: true, message: '请填写店铺名称', trigger: 'blur' }, { min: 2, max: 12, message: '店铺名称长度2-12个字符', trigger: 'blur' }]">
                <el-input v-model="shop.name" style="width:30%" placeholder="输入店铺名称"/>
            </el-form-item>
            <el-form-item label="店铺地址："
                ...
                <el-select v-model="shop.province_id" @change="provinceChange">
                    <el-option label="请选择省" value=""/>
                    <el-option v-for="(province, index) in provinces" :key="index" :label="province.name"
                               :value="province.id"/>
                </el-select>
            </el-form-item>
            <el-form-item
            ...
            <el-form-item
                ...
                <el-select v-model="shop.county_id" @change="countyChange">
                    ...
                    <el-option v-for="(county, index) in counties" :key="index" :label="county.name"
                               :value="county.id"/>
                </el-select>
            </el-form-item>
            <el-form-item label="店铺位置：" prop="" label-width="130px">
                <el-form-item
                    ...
                    <el-input v-model.number="shop.lng" placeholder="请输入经度" :disabled="true"/>
                </el-form-item>
                <el-form-item label="纬度" prop="lat"
                              label-width="64px" style="display:inline-block;"
                              :rules="[{required: true, message: '请选择定位', trigger: 'blur'}]">
                    <el-input v-model.number="shop.lat" placeholder="请输入纬度" :disabled="true"/>
                </el-form-item>
            ...
            <el-form-item label="营业时间：" prop="start_at" style="display: inline-block;">
                <el-time-picker v-model="shop['start_at']"
            ...
            <el-form-item label="" prop="end_at" label-width="10px" style="display: inline-block;">
                <el-time-picker v-model="shop['end_at']"
                                :picker-options="{start: '00:00',step: '00:05',end: '23:59', minTime: shop['start_at'] }"
            ...
            <el-form-item label="车主姓名：" prop="manager_name"
                          :rules="[{ required: true, message: '请输入餐主姓名', trigger: 'blur' }]">
            ...
            <el-form-item label="店铺地址：" prop="address"
                          :rules="[{ required: true, message: '请输入店铺位置', trigger: 'blur'}]">
