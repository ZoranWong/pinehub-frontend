...
export default class Order{
  constructor(order) {
    ...
    this.typeStr = this.orderTypes[order['type']];
    ...
    _.each(order['order_items'], function(item) {
      ...
      orderItem.statusStr = self.orderStatusDict[item['status']];
      self.orderItems.push(orderItem);
    });

    this.isAliPay = Math.random() > 0.5;
    this.isWechatPay = !this.isAliPay;
    this.payTypeStr = this.isAliPay ? '支付宝' : '微信支付';
    this.status = order['status'];
    this.statusStr = this.orderStatusDict[order['status']];
